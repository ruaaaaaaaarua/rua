...
- Follow-up retrieval is capped at two extant, current-revision, confirmed related questions. It is omitted from solves, hints, and unrevealed-question follow-ups. Public history is sanitized; every assistant response keeps its own internal audit containing only core IDs/versions, history IDs/revisions, provider profile/model identity, and prior context message IDs (never credentials). If a source is revised/deleted or a core version changes, that response and every later response that depended on it are removed from future model context; current valid history is retrieved afresh.
