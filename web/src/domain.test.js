...
import {
  ...
  clampConversationHeight,
  groupKnowledge,
  knowledgeState,
  parseSseFrames,
  questionProgress,
  savedConversationHeight,
  ...
  it("re-clamps the current height when the viewport resize callback runs", () => {
    ...
    const target = {
      ...
      addEventListener: (type, listener) => {
        if (type === "resize") callback = listener;
      },
      removeEventListener: () => {},
    };
    bindConversationResize(target, (updater) => {
      nextHeight = updater(660);
    });
    callback();
...
describe("questionProgress", () => {
  it("counts confirmed, pending and unanswered questions", () => {
    ...
    expect(questionProgress(questions)).toEqual({
      total: 3,
      answered: 2,
      confirmed: 1,
      correct: 1,
...
describe("knowledgeState", () => {
  it("maps every backend state to a display key and falls back safely", () => {
    expect(knowledgeState("viewed")).toEqual({
      key: "neutral",
      label: "已查看解析",
    });
    expect(knowledgeState("reviewed").key).toBe("success");
    expect(knowledgeState("answered").key).toBe("neutral");
    expect(knowledgeState("未知状态")).toEqual({
      key: "unknown",
      label: "未知状态",
    });
    expect(knowledgeState(undefined)).toEqual({
      key: "unknown",
      label: "尚无记录",
    });
  });
...
describe("groupKnowledge", () => {
  it("groups rows by subject and chapter while preserving unclassified values", () => {
    ...
    expect(groupKnowledge(rows)).toEqual([
      {
        subject: "继电保护",
        chapters: [{ chapter: "距离保护", items: rows.slice(0, 2) }],
      },
      {
        subject: "未分类",
        chapters: [{ chapter: "其他", items: rows.slice(2) }],
      },
    ]);
  });
});
...
describe("analysisStage", () => {
  it("distinguishes recognition from post-recognition analysis", () => {
    ...
    expect(analysisStage("analyzing").title).toBe("正在解题与检索知识库");
  });
});
...
describe("analysisRunKey", () => {
  it("changes when a later upload creates another recognized batch in the same session", () => {
    const first = analysisRunKey({
      id: "session",
      attachments: [{ id: "one", extracted: true }],
      questions: [{ id: "q1" }],
    });
    const second = analysisRunKey({
      id: "session",
      attachments: [
        { id: "one", extracted: true },
        { id: "two", extracted: true },
      ],
...
describe("parseSseFrames", () => {
  it("keeps an incomplete frame buffered and returns each completed event", () => {
    ...
    const completed = parseSseFrames(
      partial.rest + first.slice(18) + 'event: done\ndata: {"type":"done"}\n\n',
    );
